...
def random_card(cards)
  (1..cards[:deck].values.sum).to_a.sample
end
...
def dealer_face_up_card(cards)
  cards_in_hand_d = cards[:dealer_cards].select {|k, v| v > 0}
  cards_in_hand_d.to_a.sample[0]
end
  
  
def player_choice(cards)
  loop do
    cards_in_hand_p = cards[:player_cards].select {|k, v| v> 0}
    puts "your hand is #{cards_in_hand_p}"
    puts "dealer has a #{dealer_face_up_card(cards)} and one other card"
    puts 'hit or stay?'
    answer = gets.chomp
...
def score(cards)
  ...
  if initial_score[:player] > 21 && cards[:player_cards][:aces] > 0 
    revised_score[:player] = initial_score[:player] - 10
  else
    revised_score[:player] = initial_score[:player]
  end
  
  if initial_score[:dealer] > 21 && cards[:dealer_cards][:aces] > 0 
